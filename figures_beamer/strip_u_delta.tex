...
\draw[->, semithick, visible on=<3->] (P) -- ({\ta+1.2*\nxa},{\ya+1.2*\nya}) node[right] {$\vect{n}(\projvariete(x))$};

\def\eps{0.1}
\draw[semithick, visible on=<3->]
  (P)
  --
  ($(P)!-\eps cm!({\ta+\txa},{\ya+\tya})$)
  --
...
\draw[myred, thick, <->, visible on=<3->] (P) -- (\xx,\yx) node[midway, right] {$d(x)$};
...
\draw[<->, semithick, visible on=<2->] (0,\yO) -- (\xd,\yd) node[midway, left] {$\delta$};
\draw[<->, semithick, visible on=<2->] (0,\yO) -- (\xdb,\ydb) node[midway, left] {$\delta$};

\node[myblue, above=10pt, visible on=<2->] at (1,{\f(1)}) {$U_\delta$};
