...
\filldraw[fill=diamondPythonGreen, draw=diamondPythonLimeGreen, opacity=0.8] (xle2D) -- (xk2D) -- (xke2D) -- (xl2D) -- cycle;
...
\node[below] at ($(Inter)+({0.5*(\angA+\angB)}:\rayonangle)$) {$\angle{\diamantplat{D}}$};
