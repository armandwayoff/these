...
\draw[mailledualeint, visible on=<2->] (M1) -- (G1) -- (M2) -- (G2) -- (M3) -- (G3) -- (M4) -- (G4) -- (M5) -- (G5) -- cycle;
...
\node[myblue, above=6pt, visible on=<2->] at (fO) {\contour{mylightred!50!white}{$x_{\mailledualeplate{K}}$}};
\node[above=5pt, visible on=<2->] at (M2) {\contour{mylightred!50!white}{$x_\sigma$}};
\node[myblue] at ($(fO)!0.8!(fF2)$) {\contour{white}{$\sigma_{\primalplat{K},\primalplat{L}}$}};
\node[myred, below=5pt, visible on=<2->] at (G1) {$x_{\primalplat{L}}$};
% \node[myred, above=5pt] at (fG1) {\contour{white}{$x_{\primal{L}}$}};
\node[myred, below=5pt, visible on=<2->] at (G2) {$x_{\primalplat{K}}$};
...
\node[myred, below=5pt, visible on=<2->] at ($(G2)!0.5!(M2)$) {$\mailledualeplate{K}$};
