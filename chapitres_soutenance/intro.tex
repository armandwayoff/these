...
\begin{itemize}
    \item Inadaptées aux équations qui ne sont pas des bilans de conservation
    \item Difficile à appliquer aux maillages localement raffinés par exemple
\end{itemize}
\end{alertblock}
...
Conservation des identités des opérateurs au \textbf{niveau discret}
\end{center}
\end{boite}
}
\bigskip
...
\begin{enumerate}
    \only<1>{\item \fauxhighlighttxt{Généralisation et formalisation \ddfv sur les surfaces fermées de $\R^3$}}
    \only<2->{\item \highlighttxt{Généralisation et formalisation \ddfv sur les surfaces fermées de $\R^3$}}
    \item \ Démonstration de convergence
    \begin{itemize}
...
\begin{itemize}
    ...
    \item Expérimentations numériques de la méthode \ddfv sur la sphère
...
\begin{itemize}
    \item En 2D : $\surfpoly \subset \surflisse$, \highlightrougetxt{en 3D : $\surfpoly \not\subset \surflisse$}
...
\begin{column}{0.5\textwidth}
    \centering
    \input{figures_beamer/tore_poly_vs_lisse.tex}
\end{column}
\end{columns}
}
\end{overprint}
