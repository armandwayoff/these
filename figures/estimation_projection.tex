...
\draw[myblue] (0.5,0.2) -- (0.5,{\f(\xL)+0.2}) node[midway, above] {$\primal{K} = \projvariete \primalplat{K}$};
\draw[myblue] (0,0) -- (\xL,{\f(\xL)}) node[midway, below] {$\primalplat{K}$};
...
\draw[myred] (P) -- (\xx, 0) node[midway, right] {$d(x)$};
